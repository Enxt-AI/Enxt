...
export function About() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/ai-film/process");
        const data = await res.json();
        if (Array.isArray(data)) setProcessSteps(data);
      } catch (err) {
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="relative bg-gradient-to-r from-gray-950 via-gray-900 to-gray-950 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="relative py-12 px-8 overflow-hidden h-80 max-w-full">
              <div
                className={`flex transition-transform hover:pause items-center ${animationStarted ? "film-scroll-animation" : ""}`}
              ...
              >
                ...
                {processSteps.map((step, index) => (
                  <div
                    key={step.id}
                    className={`flex-shrink-0 w-80 h-52 bg-background rounded-lg border-4 ${activeFrame >= index ? `border-${getColor(index)}` : "border-gray-600"}`}
                  >
                    <div className="relative h-full p-6 flex flex-col justify-between">
                      ...
                      <div className="opacity-100 mt-2">
                        <h3 className="font-black text-xl leading-tight mb-2 text-foreground">
                          {step.title}
                        </h3>
